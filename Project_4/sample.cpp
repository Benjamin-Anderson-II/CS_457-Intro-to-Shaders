...
//    glActiveTexture(GL_TEXTURE0 + 5);
    ...
    Texture.SetUniformVariable((char *)"SkyBox", 5);
    glCallList(BoxList);
    Texture.UnUse();
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP, CubeMapTexture);
    glActiveTexture(GL_TEXTURE0 + uNoiseUnit);
    glBindTexture(GL_TEXTURE_3D, uNoiseUnit);

    Pattern.SetUniformVariable((char *)"uReflectUnit", uReflectUnit);
    Pattern.SetUniformVariable((char *)"uRefractUnit", uRefractUnit);
    Pattern.SetUniformVariable((char *)"uMix", Mix);
...
InitGraphics( )
{
    ...
    Pattern.SetUniformVariable((char *)"uEta", 1.3);
    Pattern.SetUniformVariable((char *)"uWhiteMix", 0.3);


    /* CUBE WALLS SHADER */
    Texture.Init();
    if(!Texture.Create((char *)"texture.vert", (char *)"texture.frag"))
    ...
    int nums, numt, nump;
    unsigned char *texture = ReadTexture3D((char *)"noise3d.064.tex", &nums, &numt, &nump);
    if(!texture)
        fprintf(stderr, "Could not read texture\n");
    else
        fprintf(stderr, "Noise Texture Read\n");
    ...
    for(int file = 0; file < 6; file++) {
        ...
        if(!texture2d)
            ...
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + file, 0, GL_RGB, nums, numt, 0, GL_RGB, GL_UNSIGNED_BYTE, texture2d);
        }
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
        ...
        case 'm':
            Mix = (Mix < 1) ? Mix + 0.05 : Mix;
            break;
        case 'M':
            Mix = (Mix > 0) ? Mix - 0.05 : Mix;
            break;
